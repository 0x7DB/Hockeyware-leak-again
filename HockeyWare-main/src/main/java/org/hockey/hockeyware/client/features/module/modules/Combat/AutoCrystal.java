...
import org.hockey.hockeyware.client.manager.ModuleManager;
import org.hockey.hockeyware.client.mixin.mixins.accessor.ICPacketUseEntity;
...
import org.hockey.hockeyware.client.mixin.mixins.accessor.IPlayerControllerMP;
...
public class AutoCrystal extends Module {
    ...
    public static Setting<YawStep> yawStep = new Setting<>("YawStep", YawStep.FULL);
    public static Setting<Double> yawStepThreshold = new Setting<>("YawStepThreshold", 110.0, 0.0, 180.0);
    public static Setting<Boolean> raytrace = new Setting<>("Raytrace", false);

    public static Setting<Double> offset = new Setting<>("Offset", 2.0, 0.0, 2.0);

    //public static Setting<Boolean> explode = new Setting<>("Explode", true);
    public static Setting<Double> explodeSpeed = new Setting<>("ExplodeSpeed", 19.0, 1.0, 20.0);
    public static Setting<Double> attackDelay = new Setting<>("AttackDelay", 1.8, 0.0, 5.0);
    public static Setting<Double> explodeRange = new Setting<>("ExplodeRange", 5.0, 1.0, 6.0);
    public static Setting<Double> explodeWallRange = new Setting<>("ExplodeWallRange", 3.0, 1.0, 6.0);
    public static Setting<Boolean> rangeEye = new Setting<>("RangeEye", false);
    public static Setting<Double> ticksExisted = new Setting<>("TicksExisted", 2.0, 0.0, 5.0);
    public static Setting<Double> explodeSwitchDelay = new Setting<>("SwitchDelay", 0.0, 0.0, 10.0);
    public static Setting<Inhibit> inhibit = new Setting<>("Inhibit", Inhibit.SEMI);
    public static Setting<Double> inhibitFactor = new Setting<>("inhibitFactor", 0.0, 1.0, 5.0);
    public static Setting<Boolean> await = new Setting<>("Await", true);
    public static Setting<Double> yieldProtection = new Setting<>("YieldProtection", 1.0, 0.0, 5.0);
    ...
    public static Setting<Double> placeRange = new Setting<>("PlaceRange", 4.5, 1.0, 6.0);
    public static Setting<Double> placeWallRange = new Setting<>("PlaceWallRange", 3.0, 1.0, 6.0);
    public static Setting<InventoryManager.Switch> autoSwitch = new Setting<>("Switch", InventoryManager.Switch.NORMAL);
    public static Setting<InventoryManager.Switch> antiWeakness = new Setting<>("AntiWeakness", InventoryManager.Switch.NORMAL);
    public static Setting<Boolean> alternativeSwitch = new Setting<>("AlternativeSwitch", false);
    public static Setting<Double> damage = new Setting<>("Damage", 2.0, 2.0, 10.0);
    public static Setting<Double> lethalMultiplier = new Setting<>("LethalMultiplier", 3.5, 0.0, 5.0);
    public static Setting<Boolean> armorBreaker = new Setting<>("ArmorBreaker", true);
    public static Setting<Double> armorScale = new Setting<>("ArmorScale", 0.0, 0.0, 40.0);
    public static Setting<Safety> safety = new Setting<>("Safety", Safety.STABLE);
    public static Setting<Double> safetyBalance = new Setting<>("SafetyBalance", 2.0, 0.1, 3.0);
    ...
    public static Setting<Double> targetRange = new Setting<>("TargetRange", 4.5, 0.0, 15.0);
    // **************************** render settings ****************************
    ...
    public void onUpdate() {
        ...
        if (crystalTimer.passedTime(1, Timer.Format.SECONDS)) {
        ...
        if (rotateTicks <= 0) {
            ...
            if (inhibit.getValue().equals(Inhibit.FULL)) {
                ...
                if (latestSpawn != null) {
                    ...
                    boolean delayed = System.currentTimeMillis() - latestSpawn.getKey() >= explodeDelay && switchTimer.passedTime(switchDelay, Timer.Format.MILLISECONDS);
            ...
            if (explosion != null) {
                ...
                // we have waited the proper time ???
                boolean delayed = explodeTimer.passedTime(explodeDelay, Timer.Format.MILLISECONDS) && switchTimer.passedTime(switchDelay, Timer.Format.MILLISECONDS);
        ...
        if (interact.getValue().equals(Interact.STRICT) && inhibit.getValue().equals(Inhibit.FULL)) {
            ...
            if (isDesynced()) {
                ...
                if (desyncTimer.passedTime(5, Timer.Format.SECONDS)) {
        ...
        if (rotateTicks <= 0) {
            ...
            if (attackDelay.getValue() <= attackDelay.getMin().doubleValue() || inhibitFactor.getValue() > inhibitFactor.getMin().doubleValue() && inhibit.getValue().equals(Inhibit.FULL)) {
                ...
                if (explosion != null) {
                    ...
                    // place delay based on place speeds
                    long explodeDelay = (long) ((explodeSpeed.getMax().doubleValue() - explodeSpeed.getValue()) * 50);
            ...
            if (placement != null) {
                ...
                long switchDelay = explodeSwitchDelay.getValue().longValue() * 25L;
                // we have waited the proper time ???
                boolean delayed = explodeTimer.hasReached(explodeDelay) && switchTimer.hasReached(switchDelay);
...
//        if (placement != null)
//            if (render.getValue()) {
//                RenderUtil.drawFilledBox(new AxisAlignedBB(placement.damageSource), ClickGUI.color.getValue().getRGB());
//            }
//
//    }
            ...
            if (isOn()) {
                ...
                if (angleVector != null) {
                    ...
                    if (!yawStep.getValue().equals(YawStep.NONE)) {
                        ...
                        org.hockey.hockeyware.client.util.player.Rotation serverRotation = getHockey().getRotationManager.getServerRotation();
                        ...
                        if (Math.abs(angleDifference) > yawStepThreshold.getValue()) {
                            ...
                            if (yawStep.getValue().equals(YawStep.FULL) || (yawStep.getValue().equals(YawStep.SEMI) && angleVector.second().equals(YawStep.FULL))) {
                                ...
                                getHockey().getRotationManager.setRotation(rotateAngles);
...
//    }
    ...
    public DamageHolder<EntityEnderCrystal> getCrystal() {
        ...
        for (Entity crystal : new ArrayList<>(mc.world.loadedEntityList)) {
            ...
            for (Entity entity : new ArrayList<>(mc.world.loadedEntityList)) {
                ...
                // make sure the entity actually exists
                if (entity == null || entity.equals(mc.player) || entity.getEntityId() < 0 || EnemyUtil.isDead(entity) || getHockey().friendManager.isFriend(entity.getUniqueID())) {
                    continue;
                }
                ...
                if (DamageUtil.canTakeDamage()) {
                    ...
                    // local health
                    double health = PlayerUtil.getHealth(entity);
    ...
    public boolean attackCrystal(int in) {
        ...
        // tool slots
        int swordSlot = getHockey().getInventoryManager.searchSlot(ItemSword.class, InventoryManager.InventoryRegion.INVENTORY);
        int pickSlot = getHockey().getInventoryManager.searchSlot(ItemPickaxe.class, InventoryManager.InventoryRegion.INVENTORY);
            ...
            if (weaknessEffect != null && (strengthEffect == null || strengthEffect.getAmplifier() < weaknessEffect.getAmplifier())) {
                ...
                if (!InventoryUtil.isHolding(ItemSword.class) || !InventoryUtil.isHolding(ItemPickaxe.class)) {
                    ...
                    if (antiWeakness.getValue().equals(InventoryManager.Switch.PACKET) && alternativeSwitch.getValue()) {
                        ...
                        // prefer the sword over a pickaxe
                        if (swordSlot != -1) {
                            getHockey().getInventoryManager.switchToSlot(swordSlot, antiWeakness.getValue());
                        }

                        else if (pickSlot != -1) {
                            getHockey().getInventoryManager.switchToSlot(pickSlot, antiWeakness.getValue());
                        }
                    }
                }
            }
        ...
        if (previousSlot != -1) {
            ...
            if (antiWeakness.getValue().equals(InventoryManager.Switch.PACKET)) {
                ...
                if (alternativeSwitch.getValue()) {
                    ...
                    getHockey().getInventoryManager.switchToSlot(previousSlot, InventoryManager.Switch.PACKET);
                }
            }
        }
    ...
    public boolean placeCrystal(BlockPos in) {
        ...
        int swapSlot = getHockey().getInventoryManager.searchSlot(Items.END_CRYSTAL, InventoryManager.InventoryRegion.HOTBAR) + 36;
                ...
                // switch to a crystal
                    ...
                    if (autoSwitch.getValue().equals(InventoryManager.Switch.NORMAL)) {
                        ...
                        if (autoSwitchTimer.passedTime(500, Timer.Format.MILLISECONDS)) {
                            ...
                            // switch
                            getHockey().getInventoryManager.switchToItem(Items.END_CRYSTAL, InventoryManager.Switch.NORMAL);
                        }
                    }
        ...
        Vec3d placeVector = AngleUtil.getVectorForRotation(new org.hockey.hockeyware.client.util.math.Rotation(vectorAngles.getYaw(), vectorAngles.getPitch()));
        ...
        if (autoSwitch.getValue().equals(InventoryManager.Switch.PACKET)) {
            ...
            // switch back
                ...
                if (previousSlot != -1) {
                    getHockey().getInventoryManager.switchToSlot(previousSlot, InventoryManager.Switch.NORMAL);
                }
            }
        }
        ...
        for (Entity entity : mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(
            ...
            if (entity instanceof EntityEnderCrystal) {
                ...
                for (Entity target : new ArrayList<>(mc.world.loadedEntityList)) {
                    ...
                    // make sure the entity actually exists
                    if (target == null || target.equals(mc.player) || target.getEntityId() < 0 || EnemyUtil.isDead(target) || getHockey().friendManager.isFriend(entity.getUniqueID())) {
                        continue;
                    }
